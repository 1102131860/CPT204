...
class Board {
    ...
    void setBlock(char c, char r) {
        ...
        if (ataxxBoard[index(c, r)] != BLOCKED){
            setContent(c,r,BLOCKED);
            unblockedNum --;
        }
        if (ataxxBoard[index(c, row)] != BLOCKED){
            setContent(c,row,BLOCKED);
            unblockedNum --;
        }
        if (ataxxBoard[index(col, row)] != BLOCKED){
            setContent(col,row,BLOCKED);
            unblockedNum --;
        }
        if (ataxxBoard[index(col, r)] != BLOCKED){
    ...
    PieceState getWinner() {
        ...
        // 1. Neither player can make a move (i.e. the board is full).
        // 2. There are 25 consecutive jumps without any clones.
        // 3. A player has no pieces of their color on the board.
		if(!couldMove(RED) && !couldMove(BLUE) || getConsecJumpNums() >= CONSEC_JUMP_LIMIT
            || getColorNums(RED) == 0 || getColorNums(BLUE) == 0){
